...
class _GitHubAPI: ObservableObject {
    ...
    @Published private(set) var personalAccessToken: String? {
        didSet {
    ...
    var authHeader: String? {
        personalAccessToken.map { "Bearer \($0)" }
    }
    ...
    var activeRepositories: TypedResource<[Repository]> {
        service
    ...
    func user(_ username: String) -> TypedResource<User> {
        service
        .resource("/users")
    ...
    func currentUserStarred(_ repositoryModel: Repository) -> TypedResource<Bool> {
        service
