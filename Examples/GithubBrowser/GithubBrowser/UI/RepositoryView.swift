    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading) {
                ResourceView(GitHubAPI.repository(ownedBy: owner, named: name), displayRules: [.allData, .loading, .error]) { (repository: Repository) in
                    VStack(alignment: .leading) {
                        ResourceView(GitHubAPI.currentUserStarred(repository), displayRules: [.allData]) { (isStarred: Bool) in
                            HStack {
                                Text(isStarred ? "★" : "☆")
                        ...
                        if let resource = GitHubAPI.languages(repository) {
                            ...
                            ResourceView(resource, displayRules: [.allData, .loading]) { (lang: [String: Int]) in
                        ...
                        if let resource = GitHubAPI.contributors(repository) {
                            ...
                            ResourceView(resource, displayRules: [.allData, .loading]) { (users: [User]) in
