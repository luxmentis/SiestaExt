        ...
        describe("statePublisher()") {
            it("outputs content when the request succeeds") {
                ...
                .sinkUntilTested {
                    ...
                    if case .newData = state.event { newDataEventExpectation.fulfill() }
            ...
            it("outputs content when the content is already cached") {
                ...
                resource().statePublisher()
                .compactMap { (state: ResourceState<String>) in
                    state.content != nil ? resource().statePublisher() : nil
                }
                .prefix(1)
                .switchToLatest()
                .sink { (state: ResourceState<String>) in
            ...
            it("outputs an error when the request fails") {
                ...
                .sinkUntilTested {
                    (state: ResourceState<String>) in
                    if state.latestError != nil { return true }
                    return false
                }
            }
